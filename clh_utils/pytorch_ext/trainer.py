...
class BaseTrainer:
    ...
    def train(self, epoch, history):
        ...
        widgets += [progressbar.DynamicMessage(k) for k in self.metrics_functions.keys()]
        with progressbar.ProgressBar(max_value=len(self.dataloaders['train']), widgets=widgets) as bar:
            for it, batch in enumerate(self.dataloaders['train']):
                ...
                batch = self.model.sort_batch(batch)
            ...
            s = 'Epoch {}, train, loss = {:.4f}'.format(epoch + 1, meters['loss'].avg)
            for k in self.metrics_functions.keys():
            ...
            with torch.no_grad():
                for it, batch in enumerate(self.dataloaders['val']):
                    ...
                    for k in self.metrics_functions.keys():
                        bar_values[k] = meters[k].avg
                    bar.update(it, **bar_values)
                s = 'Epoch {}, val, loss = {:.4f}'.format(epoch + 1, meters['loss'].avg)
                for k in self.metrics_functions.keys():
                    s += ', {} = {:.4f}'.format(k, meters[k].avg)
                print(s)
    ...
    def fit(self):
        if self.metrics_functions is None:
            metrics_functions = {}
        ...
        try:
            ...
            for epoch in range(self.begin_epoch, num_epochs):
                print('Starting epoch {}, lr = {}'.format(epoch + 1, get_lr(self.optimizer)))
                ...
                if lr < self.min_lr:
                    break
                if self.save_every:
                    save_model(self.model, self.optimizer, self.scheduler, epoch, self.save_model_dir, history)

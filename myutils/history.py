...
class History:
    ...
    def plot_loss(self):
        plt.figure()
        epochs = len(self.records['loss']['train'])
        plt.plot(range(epochs), self.records['loss']['train'], label='loss_train')
        plt.plot(range(epochs), self.records['loss']['val'], label='loss_val')
        plt.legend()
        plt.show()
    ...
    def plot_metrics(self):
        ...
        for key in self.records.keys():
            ...
            print(key)
            epochs = len(self.records[key]['train'])
            plt.plot(range(epochs), self.records[key]['train'], label=key + '_train')
            plt.plot(range(epochs), self.records[key]['val'], label=key + '_val')
        plt.legend()
        plt.show()
